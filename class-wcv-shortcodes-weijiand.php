...
class WCV_Shortcodes {
	...
	function __construct() {
		...
		// Display vendor by login

		add_shortcode( 'wcv_display_vendor_by_login', array( $this, 'wcv_display_vendor_by_login') );
	...
	*	featured_vendors_with_products - Get vendors with products pubilc or private 

	*	@param array $query 	

	*/

	/* public function featured_vendors_with_products( $query ) {
	    ...
	    if ( isset( $query->query_vars['query_id'] ) && 'featured_vendors_with_products' == $query->query_vars['query_id'] ) {  
	...
	/**
	  ...
	  * 	Display vendor by vendor's login username 
	...
	*/
	...
	public function wcv_display_vendor_by_login( $atts ) {
	  ...
	  /* 	$paged      = ( get_query_var('paged') ) ? get_query_var('paged') : 1;   
	  	...
	  	// Hook into the user query to modify the query to return featured users that have at least one product 

	  	/* if ($show_products == 'yes') add_action( 'pre_user_query', array( $this, 'featured_vendors_with_products') ); */
	  	...
	  	/* if ($show_products == 'yes') $vendor_total_args['query_id'] = 'featured_vendors_with_products'; */ 
	  	...
	  	/* if ($show_products == 'yes' ) $vendor_paged_args['query_id'] = 'featured_vendors_with_products';  */
